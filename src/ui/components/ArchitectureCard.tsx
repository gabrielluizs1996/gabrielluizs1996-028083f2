...
export function ArchitectureCard({
  ...
  return <article className={cn("relative pl-8 pb-8 last:pb-0", className)} aria-labelledby={`arch-${item.id}`}>
      {/* Timeline connector */}
      ...
      <div className="card-professional px-0 my-[16px]">
        <div className="flex items-baseline gap-3 mb-2">
          ...
          <h3 id={`arch-${item.id}`} className="text-lg font-semibold text-foreground">
            {item.title}
          </h3>
        </div>
